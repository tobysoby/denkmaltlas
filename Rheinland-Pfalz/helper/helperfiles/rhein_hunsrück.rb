...
url = 'http://de.wikipedia.org/wiki/Vorlage:Navigationsleiste_Kulturdenkm%C3%A4ler_im_Rhein-Hunsr%C3%BCck-Kreis'
...
doc.css('.NavContent').css('a').each do |eintrag|
    ...
    if stadtteil != "Hauptstuhl" && stadtteil != "Simmern/Hunsrück"
    ...
    kreis.css('li').each do |denkmal|
        ...
        bezeichnungen_array.each_with_index do |vitamin, index|
            CSV.open('../ergebnisse/Rhein-Hunsrück/' + stadtteil_array[index] + '.csv', 'a+') do |csv|
