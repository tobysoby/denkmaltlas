...
url = 'http://de.wikipedia.org/wiki/Vorlage:Navigationsleiste_Kulturdenkm%C3%A4ler_in_der_Verbandsgemeinde_Arzfeld'
...
doc.css('.NavContent').css('a').each do |eintrag|
    ...
    CSV.open('Arzfeld/' + stadtteil + '.csv', 'a') do |csv|
        #csv << ['bezeichnung', 'lage', 'baujahr', 'beschreibung']
        bezeichnungen_array.each_with_index do |vitamin, index|
            puts bezeichnungen_array[index], stadtteil, beschreibung_array[index], lat_array[index], long_array[index]
