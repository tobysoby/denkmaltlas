...
url = 'http://de.wikipedia.org/wiki/Liste_der_Kulturdenkm%C3%A4ler_in_Kaiserslautern'
...
#doc.css('.NavContent').css('a').each do |eintrag|
    ...
    kreis.css('li').each do |denkmal|
        ...
        bezeichnungen_array.each_with_index do |vitamin, index|
            CSV.open('../ergebnisse/Kaiserslautern_Stadt/' + stadtteil_array[index] + '.csv', 'a+') do |csv|
