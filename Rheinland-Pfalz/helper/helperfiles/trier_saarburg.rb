...
url = 'http://de.wikipedia.org/wiki/Vorlage:Navigationsleiste_Kulturdenkm%C3%A4ler_im_Landkreis_Trier-Saarburg'
...
doc.css('.NavContent').css('a').each do |eintrag|
    ...
    kreis.css('li').each do |denkmal|
        ...
        bezeichnungen_array.each_with_index do |vitamin, index|
            CSV.open('../ergebnisse/Trier-Saarburg/' + stadtteil_array[index] + '.csv', 'a+') do |csv|
